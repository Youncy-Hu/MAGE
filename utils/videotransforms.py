...
def resize_clip(clip, size, interpolation='bilinear'):
    if isinstance(clip[0], np.ndarray):
        ...
        im_c = clip[0].shape[-1]
        scaled = [
            cv2.resize(img, size, interpolation=np_inter).reshape(size[0], size[1], im_c) for img in clip
        ]
    elif isinstance(clip[0], PIL.Image.Image):
        if isinstance(size, numbers.Number):
...
class GaussianBlur(object):
    ...
    def __call__(self, clip):
        if torch.rand(1) < self.p:
            ...
            x = [cv2.GaussianBlur(img, (0, 0), sigmaX=sigma, sigmaY=sigma).reshape(im_h, im_w, im_c) for img in clip]
            return x
        return clip
